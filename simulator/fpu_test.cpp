...
int main(){
    ...
    for(int i=0; i<10; i++){     
        ...
        if(is_invalid(y)){
            ...
            std::cout << "\x1b[1m" << string_of_ftype(t) << ": \x1b[31mtoo large error\x1b[0m" << std::endl;
...
// 引数が1つかどうかを判定
inline bool is_single_arg(Ftype t){
    if(t == Ftype::o_finv || t == Ftype::o_fsqrt || t == Ftype::o_ftoi || t == Ftype::o_itof || t == Ftype::o_floor){
        return true;
    }else{
        return false;
    }
