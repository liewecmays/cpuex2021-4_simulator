...
void exec_op(){
    ...
    switch(op.type){
        ...
        case Otype::o_fmvff:
            ...
            return;
        case Otype::o_beq:
            reg_int.read_int(op.rs1) == reg_int.read_int(op.rs2) ? pc += op.imm * 4 : pc += 4;
        ...
        case Otype::o_blt:
            ...
            return;
        case Otype::o_fbeq:
            reg_fp.read_float(op.rs1) == reg_fp.read_float(op.rs2) ? pc += op.imm * 4 : pc += 4;
        ...
        case Otype::o_fblt:
            ...
            return;
        case Otype::o_sw:
            if((reg_int.read_int(op.rs1) + op.imm) % 4 == 0){
                write_memory((reg_int.read_int(op.rs1) + op.imm) / 4, reg_int.read_32(op.rs2));
            }else{
        ...
        case Otype::o_si:
            if((reg_int.read_int(op.rs1) + op.imm) % 4 == 0){
                op_list[(reg_int.read_int(op.rs1) + op.imm) / 4] = Operation(reg_int.read_int(op.rs2));
            }else{
        ...
        case Otype::o_fsw:
            if((reg_int.read_int(op.rs1) + op.imm) % 4 == 0){
                write_memory((reg_int.read_int(op.rs1) + op.imm) / 4, reg_fp.read_32(op.rs2));
            }else{
                exit_with_output("address of store operation should be multiple of 4 [sw] (at pc " + std::to_string(pc) + (is_debug ? (", line " + std::to_string(id_to_line.left.at(id_of_pc(pc)))) : "") + ")");
            }
        ...
        case Otype::o_lw:
            if((reg_int.read_int(op.rs1) + op.imm) % 4 == 0){
                reg_int.write_32(op.rd, read_memory((reg_int.read_int(op.rs1) + op.imm) / 4));
            }else{
        ...
        case Otype::o_flw:
            if((reg_int.read_int(op.rs1) + op.imm) % 4 == 0){
                reg_fp.write_32(op.rd, read_memory((reg_int.read_int(op.rs1) + op.imm) / 4));
            }else{
        ...
        case Otype::o_jalr:
            {
                ...
                pc = reg_int.read_int(op.rs1) + op.imm * 4;
                reg_int.write_int(op.rd, next_pc);
                ++op_type_count[Otype::o_jalr];
            }
            return;
        case Otype::o_jal:
            reg_int.write_int(op.rd, pc + 4);
